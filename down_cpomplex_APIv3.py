...
class extract_text():
    ...
    def save_log(self, data, errorflag):
        ...
        pathlib.Path(def_dir).mkdir(exist_ok=True, parents=True)

        if not self.tag:
            log_name = def_dir + data[0]
        else:
            log_name = def_dir + str(self.tag)
            pathlib.Path(log_name).mkdir(exist_ok=True, parents=True)
...
def main():
    ...
    if len(args) >= 2:
        ...
        elif 'https://beta.sankakucomplex.com/post/show/' in args[1]:
            print('get one post')

            post_id = args[1].replace('https://beta.sankakucomplex.com/post/show/', '')
            re.sub('\?*', '', post_id)
            extractor.get_post_one(post_id)
        else:
            print('please argv Like below')
            print('tag_name [tag:***]')
            print('post_url [https://beta.sankakucomplex.com/post/show/*****]')
            print('post_tag_text [./z_list/sankaku_favorite.txt]')
    else:
        print('please argv Like below')
        print('tag_name [tag:***]')
        print('post_url [https://beta.sankakucomplex.com/post/show/*****]')
...
if __name__ == '__main__':
    pathlib.Path(src_save_path).mkdir(exist_ok=True, parents=True)
    main()
